	...
	attach: function() {
		...
		function build() {
			...
			return function( req, res, next ) {
				...
				if ( !match || match[ 1 ] ||
					!( app = bender.applications.findOne( 'url', match[ 1 ] ) ) ) {
					return resume();
				}
				...
				if ( app.proxy ) {
					...
					req
						...
						.pipe( new Instrumenter( filePath ) )
						.pipe( res );
				} else {
					filePath = path.normalize( bender.utils.stripParams( filePath ) );

					// server file from local file system
					fs.createReadStream( path.resolve( app.path, filePath ) )
						.on( 'error', resume )
